...
*   **Dev UIでのモデル表示:** `googlegenai`プラグインには、Dev UIで利用可能なモデルを自動的に入力しないというバグがあります。モデルを名前で参照して「動的に」登録することは引き続き可能ですが、実験プロセスに少し摩擦が生じます（JSバージョンはこれをうまく行います）。私は[バグを報告](https://github.com/firebase/genkit/issues/4783)し、すでに修正が行われていますが、古いバージョンを使用している場合は注意が必要です。
