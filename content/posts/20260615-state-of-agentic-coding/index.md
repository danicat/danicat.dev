...
Instead of spinning up MCP servers, more and more people are preferring to use agent skills combined with CLI tools. While I respect that, I do have some trust issues giving my agents direct shell access and I prefer to package my tools in MCP servers that I run locally (like [GoDoctor]({{< ref "/posts/20250729-how-to-build-an-mcp-server-with-gemini-cli-and-go" >}}) or [Speedgrapher]({{< ref "/posts/20250805-introducing-speedgrapher" >}})). The only thing that changed for me is that I am much more selective in installing MCP servers, and most of the time I have only my custom made ones configured.
