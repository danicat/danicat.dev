...
ADK provides a universal launcher (`full.NewLauncher()`) that eliminates the need to manually construct HTTP routing, JSON marshalling, or session boilerplate. By passing command-line arguments to the launcher, you can run the agent across multiple interaction modes without altering application code.

To start an interactive terminal chat session, execute:
...
The launcher starts in console mode, allowing you to converse with the appraiser directly:
...
Because ADK manages conversational state and session continuity automatically, follow-up queries retain full contextual memory of earlier turns in the conversation.

Alternatively, you can launch ADK's built-in developer Web UI and REST API server:

```sh
go run main.go web webui api
```

Navigating to `http://localhost:8080` opens an interactive web chat interface that provides real-time streaming responses, session history, and transparent step-by-step visibility into tool executions (both your custom `search_catalog` function tool and native `GoogleSearch` grounding).

ADK provides clean architectural separation between agent definition (`llmagent`), tool bindings (`functiontool` and built-in tools like `geminitool.GoogleSearch`), and execution environments (`launcher` and `runner`), making it the framework of choice when building complex multi-agent hierarchies or conversational assistants.
